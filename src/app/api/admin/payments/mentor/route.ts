...
import { recordMentorPaymentSchema } from "@/lib/validation/payment.schemas";
import { ZodError } from "zod";

/**
 * POST /api/admin/payments/mentor
 * Record a payment to a mentor for a mission
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const validatedData = recordMentorPaymentSchema.parse(body);
        ...
        // Verify mission exists, belongs to mentor, and is validated
        const mission = await prisma.mission.findUnique({
            where: { id: validatedData.missionId },
            include: {
                transactions: {
                    where: { type: "MENTOR_PAYMENT" },
                    select: { amount: true },
                },
            },
        });
        ...
        if (mission.mentorId !== validatedData.mentorId) {
            return NextResponse.json(
                { error: "Mission does not belong to this mentor" },
                { status: 400 }
            );
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const transaction = await tx.transaction.create({
                data: {
                    transactionNumber: `TX-MNT-${Date.now()}`,
                    date: new Date(validatedData.paymentDate),
                    type: "MENTOR_PAYMENT",
                    amount: validatedData.amount,
                    currency: validatedData.currency,
                    description: `Payment to mentor ${mentor.user.name} for mission ${mission.title}`,
        ...
        // Handle other errors
        console.error("Error recording mentor payment:", error);
        return NextResponse.json(
